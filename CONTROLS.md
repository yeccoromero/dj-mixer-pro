...
| **Contador LED (transcurrido) / Rueda de Scratch + Botón ▶/⏸ (Play/Pause) / Contador LED (restante)** | Los tres grupos van en una sola fila, centrados entre los dos contadores (`grid-cols-[1fr_auto_1fr]`) — es el control principal del deck | El Play toca `DeckState.isPlaying` (lo actualiza el propio evento `onStateChange` del reproductor, no el clic directamente); los contadores solo leen `DeckState.currentTime`/`track.duration`, son informativos; la rueda de Scratch pausa/seekea/reanuda directamente sobre el `player`, sin pasar por ningún campo nuevo de `DeckState` | Play deshabilitado hasta que el reproductor emite `onReady`; la rueda además necesita una pista asignada |
| **Rueda de Scratch** *(`ScratchWheel.tsx`, al lado izquierdo del Play)* | Arrastrarla gira la rueda libremente (sin límite de ángulo) y mueve la pista con el gesto: mientras se sostiene, la pista queda pausada y "salta" según cuánto se giró; al soltar, retoma la reproducción si estaba sonando antes (o queda pausada ahí, si ya estaba pausada) | Ninguno en `DeckState` — llama directo a `player.pauseVideo()`/`seekTo()`/`playVideo()` y a `onStateChange` solo para reflejar `currentTime` en vivo | Ver nota abajo sobre por qué esto es una aproximación del gesto, no un scratch de audio real |
...
**Scratch — por qué es una aproximación del gesto, no un scratch de audio real:** pedido
explícito ("un scratch, ¿cómo lo ponemos?"). Un scratch de verdad necesita control de audio en
tiempo real — velocidad variable, reproducción inversa, todo sonando sin cortes — pero la pista
suena a través del iframe de YouTube, que solo expone `seekTo`/`play`/`pause` (sin acceso al
buffer de audio ni reproducción en reversa), la misma limitación que ya impidió un filtro real de
audio (ver "Filter" más abajo en este documento). En vez de descartar la idea, se implementó la
aproximación más honesta posible dentro de esa limitación: `ScratchWheel.tsx` es una rueda libre
(GSAP `Draggable type: "rotation"`, sin límite de ángulo y **sin inercia** — a diferencia del
viejo knob de Gain, acá no tiene sentido que la rueda "siga girando sola" al soltar, porque nada
sigue sonando durante ese giro) que, mientras se arrastra, pausa la pista y hace `seekTo` según
cuánto giró (`SCRATCH_SECONDS_PER_REVOLUTION` en `Deck.tsx`: una vuelta completa mueve 6 segundos
de pista) — el resultado es un "tartamudeo" audible, no el sonido de scratch real, pero sí el
mismo *gesto* de mover la pista con la mano hacia adelante/atrás. Al soltar, retoma la
reproducción normal si estaba sonando, o queda pausada en el nuevo punto si ya lo estaba. Los
saltos de posición se limitan a cada 0.05s de movimiento acumulado (`SCRATCH_SEEK_THRESHOLD_SECONDS`)
en vez de en cada píxel de arrastre — el evento `onDrag` de GSAP dispara mucho más seguido que el
polling normal de la app (200ms), y llamar a `seekTo` en cada tick saturaría el puente
`postMessage` hacia el iframe sin ganancia perceptible; el punto final del gesto siempre aterriza
exacto igual, aunque el último movimiento haya quedado por debajo del umbral.
